...
public interface PvMHUDConfig extends Config {
    ...
    @ConfigSection(name = "General", description = "Choose the HUD style and visible indicators.", position = 0)
    String generalSection = "general";

    @ConfigSection(name = "Text and icon HUD", description = "Configure the Text and Game icons HUD styles.", position = 1)
    String textIconSection = "textIcon";

    @ConfigSection(name = "Bars and compact HUDs", description = "Configure Bars, Chips, Orbs, and Stack HUD styles.", position = 2)
    String barStyleSection = "barStyle";

    @ConfigSection(name = "Above-head alerts", description = "Configure local-only above-head alerts.", position = 3)
    String alertSection = "alerts";

    @ConfigSection(name = "Spell timing", description = "Configure indicator timing and warnings.", position = 4)
    String timingSection = "timing";

    @ConfigSection(name = "Stat thresholds", description = "Configure HP, Prayer, and special attack thresholds.", position = 5)
    String thresholdSection = "thresholds";

    @ConfigSection(name = "Background", description = "Configure the HUD background.", position = 6)
    String backgroundSection = "background";

    @ConfigSection(name = "Stat colors", description = "Set HP, Prayer, and special attack colors.", position = 7)
    String statColorSection = "statColors";

    @ConfigSection(name = "Spell state colors", description = "Set shared ready, cooldown, expiry, and flash colors.", position = 8)
    String spellStateColorSection = "spellStateColors";

    @ConfigSection(name = "Active spell colors", description = "Set active colors for each tracked spell.", position = 9)
    ...
    @ConfigItem(keyName = "overheadHpAlertEnabled", name = "HP alert",
            description = "Show a local above-head alert when Hitpoints crosses below the low threshold.",
            position = 0, section = alertSection)
    default boolean overheadHpAlertEnabled() { return true; }

    @ConfigItem(keyName = "lowHpOverheadMessage", name = "HP message",
            description = "Above-head message for low Hitpoints.",
            position = 1, section = alertSection)
    default String lowHpOverheadMessage() { return "Low HP!"; }

    @ConfigItem(keyName = "overheadPrayerAlertEnabled", name = "Prayer alert",
            description = "Show a local above-head alert when Prayer crosses below the low threshold.",
            position = 2, section = alertSection)
    default boolean overheadPrayerAlertEnabled() { return true; }

    @ConfigItem(keyName = "lowPrayerOverheadMessage", name = "Prayer message",
            description = "Above-head message for low Prayer.",
            position = 3, section = alertSection)
    default String lowPrayerOverheadMessage() { return "Low Prayer!"; }

    @ConfigItem(keyName = "overheadSpecAlertEnabled", name = "Spec alert",
            description = "Show a local above-head alert when special attack crosses above the configured threshold.",
            position = 4, section = alertSection)
    default boolean overheadSpecAlertEnabled() { return false; }

    @ConfigItem(keyName = "specOverheadMessage", name = "Spec message",
            description = "Above-head message for special attack threshold.",
            position = 5, section = alertSection)
    ...
    @ConfigItem(keyName = "deathChargeCooldownColor", name = "Death Charge consumed",
            description = "Color after Death Charge restores spec but before the effect window ends.",
            position = 3, section = spellStateColorSection)
    ...
    @ConfigItem(keyName = "backgroundColor", name = "Color", description = "HUD background color.", position = 0, section = backgroundSection)
    default Color backgroundColor() { return new Color(12, 16, 22); }

    @Range(min = 0, max = 255)
    @ConfigItem(keyName = "backgroundAlpha", name = "Opacity", description = "Background opacity. Use 0 for no background.", position = 1, section = backgroundSection)
